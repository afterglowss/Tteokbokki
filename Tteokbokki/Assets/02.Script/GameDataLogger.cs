...
public class GameDataLogger : MonoBehaviour
{
    ...
    public void SaveDailyLog()
    {
        ...
        if (!File.Exists(filePath) || new FileInfo(filePath).Length == 0)
        {
            ...
            // 재료 헤더는 동적으로 생성하지 않고, 일단 Used/Sold/Waste/Bought 반복된다고 가정하거나
            // 주요 재료들만 고정해서 넣는 게 좋지만, 여기선 수집된 순서대로 기록합니다.
            // (실제 분석 시엔 DB의 모든 키를 기준으로 헤더를 미리 만드는 게 좋습니다)
        ...
        foreach (var key in sortedKeys)
        {
            ...
            {
                sb.Append("0,0,0,0,"); // 데이터 없으면 0 처리
            }
        }
        sb.AppendLine();
    ...
    {
        // DB에 있는 모든 키를 가져와서 정렬 (항상 같은 컬럼 순서 유지를 위해)
